...
module LoopStack #(parameter ADDR_WIDTH = 4, parameter ROM_ADDR_WIDTH = 16, parameter WIDTH = 16) (
    input wire rst,
    input wire clk,
    input wire new, // enabled to create a new loop using 'pc_in' and 'cnt_in'
    input wire [WIDTH-1:0] cnt_in, // number of iterations in loop created when 'new'
    input wire [ROM_ADDR_WIDTH-1:0] pc_in, // the program counter at 'loop' op
    input wire nxt, // enabled if instruction is also 'next'
    input wire done_ack, // enabled if cpu acknowledged the 'done' at current 'next' instruction
    output reg [ROM_ADDR_WIDTH-1:0] pc_out, // the 'pc_in' when the loop was created, stable during negative edge
    output reg done // enabled if loop is at last iteration, stable during negative edge
    );

reg [ROM_ADDR_WIDTH-1:0] stk_addr [0:2**ADDR_WIDTH-1]; // stack of loop begin addresses
